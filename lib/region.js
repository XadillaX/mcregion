...
Region.prototype.initializeChunks = function(callback) {
    ...
    for(var i = 0; i < this._chunkInformation.length; i++) {
        ...
        this._chunkInformation[i].chunk        = new Chunk(
            this._chunkInformation[i].originalData
        );

        scarlet.push({
            scarlet: scarlet,
            info: this._chunkInformation[i]
        }, this._parseChunk.bind(this));
    }
    ...
    scarlet.afterFinish(this._chunkInformation.length, function() {
        for(var i = 0; i < self._chunkInformation.length; i++) {
            if(!(self._chunkInformation[i].chunk instanceof Chunk)) {
                return callback(
                    new Error("Broken chunk at " + i + ": " +
                              self._chunkInformation[i].chunk.message + "."));
            }
        }
...
Region.prototype._init = function() {
    ...
    for(var i = 0; i < 4096; i += 4) {
        var sectorOffset = (this._buffer.readUInt8(i) << 16) +
            (this._buffer.readUInt8(i + 1) << 8) +
            this._buffer.readUInt8(i + 2);
        var sectorCount  = this._buffer.readUInt8(i + 3);

        this._chunkInformation.push({
    ...
    for(var i = 4096; i < 8192; i += 4) {
        ...
        var idx = (i - TIMESTAMP_BASE_OFFSET) / 4;
        this._chunkInformation[idx].timestamp = timestamp;
    }
};
